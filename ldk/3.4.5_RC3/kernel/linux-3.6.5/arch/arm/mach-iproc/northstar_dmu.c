...
//#ifdef CONFIG_HW_REBOOT_5334X	

	/*cmic_gp_out_en*/
	int outEn = ioremap(0x48002008, 4); 
	/*cmic_gp_data_out*/
	int dateOut = ioremap(0x48002004, 4); 

	//printk("read kernel data %d in northstar_restart\n", *((int*)outEn));
	//printk("read kernel data %d in northstar_restart\n", *((int*)dateOut));
	/* set gpio register to restart */
	*((int*)dateOut) &= (~4);
    *((int*)outEn) |= 4;
//#endif	
