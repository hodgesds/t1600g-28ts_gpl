	...
	if (stage_session(sp->sa_addr.pppoe.sid)) {
		...
		if (error) {
			...
			goto err_put;
...
#ifdef CONFIG_IPROC_FA2
#ifdef CONFIG_IPROC_PPPOE_FA
                printk(KERN_INFO "\n*** FA+ pppoe session ID: 0x%x\n", po->pppoe_pa.sid);
                printk(KERN_INFO "*** FA+ pppoe remote: %x:%x:%x:%x:%x:%x\n",
                ...
                printk(KERN_INFO "*** FA+ pppoe dev: %s, MAC: %x:%x:%x:%x:%x:%x\n",
                                                          po->pppoe_pa.dev,
                                                          *dev->dev_addr,
                                                          *(dev->dev_addr+1),
                                                          *(dev->dev_addr+2),
                ...
                fa2_setup_pppoe_tunnel(po->pppoe_pa.sid>>8,
                                (char *)po->pppoe_pa.remote, 
                                (char *)dev->dev_addr);
#endif
#endif
