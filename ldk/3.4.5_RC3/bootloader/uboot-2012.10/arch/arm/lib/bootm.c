...
static void boot_prep_linux(bootm_headers_t *images)
{
	...
	MESSAGE("%s commandline: %s\n", __FUNCTION__, commandline);
#ifdef CONFIG_OF_LIBFDT
	if (images->ft_len) {
		debug("using: FDT\n");
		if (create_fdt(images)) {
			MESSAGE("FDT creation failed! hanging...");
			hang();
		}
	} else
#endif
	...
	defined(CONFIG_REVISION_TAG)
		...
		setup_end_tag(gd->bd);
#else /* all tags */
		MESSAGE("FDT and ATAGS support not compiled in - hanging\n");
		hang();
#endif /* all tags */
	}
}
...
static void boot_jump_linux(bootm_headers_t *images)
{
	...
	debug("## Transferring control to Linux (at address %08lx) machid=%0x" \
    ...
    debug("## Transferring control to Linux (at address %08lx) machid=%0x r2=%0x" \
