...
#if (defined(CONFIG_TP_WOLFHOUND_BOARD) || defined(CONFIG_TP_HELIX4_BOARD))
#undef CONFIG_RUN_DDR_SHMOO2
#undef CONFIG_SHMOO_REUSE
#endif
...
#if defined(CONFIG_TP_WOLFHOUND_BOARD)
extern unsigned int ddr2_init_tab_800_tp_wolfhound_board[];
extern unsigned int ddr2_init_tab_667_tp_wolfhound_board[];
extern unsigned int ddr2_init_tab_1066_tp_wolfhound_board[];
extern unsigned int ddr3_init_tab_800_tp_wolfhound_board[];
#endif
...
#if defined(CONFIG_TP_HELIX4_BOARD)
extern unsigned int ddr3_init_tab_800_tp_helix4_board[];
#endif
...
extern unsigned int ddr_phy_wl0_tab_ddr2[];
extern unsigned int ddr_phy_wl0_tab_ddr3[];
...
uint32_t iproc_get_ddr3_clock_mhz(uint32_t unit)
{
	...
	if(ddrclk == 466 || ddrclk == 666)
		ddrclk +=1;
	MESSAGE("%s ndiv(0x%x) mdiv(0x%x) ddrclk(0x%x)\n", __FUNCTION__, ndiv, mdiv, ddrclk);
	return(ddrclk);
}
...
uint32_t change_ddr_clock(uint32_t clk)
{
	...
	while(1) {
		...
		if(ddr_clk_tab[i] == 0xffffffff) {
			MESSAGE("Error in change_ddr_clock: Can't find clock configuration requested\n");
			return(1);
		}
		i += 3;
	}
...
int ReWriteModeRegisters( void )
{
    ...
    if ( j == 0 && (reg32_read( (volatile uint32_t *)DDR_DENALI_CTL_89) & (1 << 18) ) == 0 ) {
        MESSAGE("Error: DRAM mode registers write failed\n");
        nRet = 1;
    };
...
int is_ddr_32bit(void)
{
	...
	if (!status) {
		//MESSAGE("OTP[0xc]: 0x%x\n", otp_status);
		if ( ((otp_status>>21)&0x07ff) == 0x25) {
			//MESSAGE("NSP25 32bit DDR\n");
			ddr32=1;
		}
	}
#endif /* (defined(CONFIG_NS_PLUS)) */
...
void ddr_init(void)
{
	...
	if (dev_id >= 0xcf19)	 {	/* 53017 */
		sku_id =0;
		MESSAGE("SKU ID (CoStar) = %d\n", sku_id);
	}

	status = is_otp_programmed(&otp_status);
	if(status) {
		MESSAGE("OTP status check failed\n");
		return;
	}

	MESSAGE("OTP status: %x\n", otp_status);
...
#endif
	MESSAGE("MEMC 0 DDR speed = %dMHz\n", ddr_clk);
	status = change_ddr_clock(ddr_clk);
	if(status) {
		MESSAGE("CRU LCPLL configuratioin failed\n");
		return;
	}
...
/*DDR_SHMOO_RELATED_CHANGE: change clock from 400 to ddr_clk*/    
	...
	if(status != DDR40_PHY_RETURN_OK) {
		MESSAGE("Error: ddr40_phy_init failed with error 0x%x\n", status);
		return;
	}
	...
	/* mhz = 0x190(400MHz), 32 bit bus, 16 bit chip width, 1.50v vddq, 2Gbit chip size, jedec type = 16 (DDR3-1333H) */
	reg32_write((volatile uint32_t *)DDR_PHY_CONTROL_REGS_STRAP_CONTROL, 0x01904561);
	...
	/* ECC =0, DRAM type = 1 i.e DDR3, AL = 0, write recovery = 0, write latency = 6, CAS read latency = 6 */	
	reg32_write((volatile uint32_t *)DDR_PHY_CONTROL_REGS_STRAP_CONTROL2, 0x00100306);
	...
	if(i == 0x1400) {
		MESSAGE("DDR PHY PLL lock failed\n");
		return;
	}
	...
	if(i == 0x1400) {
		MESSAGE("DDR PHY auto calibration timed out\n");
		return;
	}
...
calib_override:
		MESSAGE("Auto calibration failed, do the override\n");
...
ddr_cntrl_prog:
#ifdef CONFIG_RUN_DDR_SHMOO
	MESSAGE("PHY register dump after DDR PHY init\n");
	dump_phy_regs();
#endif
...
/*DDR_SHMOO_RELATED_CHANGE: mark out mode register update as it should already be programmed before "turnon"*/
	...
	MESSAGE("PHY register dump after mode register write\n");
	dump_phy_regs();
...
/*DDR_SHMOO_RELATED_CHANGE: chanhe shmoo memory test base from 0x0 (write-through) to 0x1000000 (write-back)*/
	...
	if(status != SHMOO_NO_ERROR) {
		MESSAGE("Error: do_shmoo failed with error 0x%x\n", status);
		return;
	}
		...
		*(unsigned int *)(0x1800c154) = 0x83c00000;     // write bit31=1 , sel_SW_setting
		...
		if( tmp & (1<<4) ) { //bit 4 is set as 1 by "shmoo" command
			...
			*(unsigned int *)(0x1800c154) = 0x83c00000;               // write bit31=1 , sel_SW_setting
			...
			MESSAGE ("Forcing to MDE entry at %08x\n", (unsigned) mde_entry );
			mde_entry ();
		}
	}
...
static int write_shmoo_to_flash(void *buf, int length)
{
    ...
    if (nand->size < offset || nand->writesize == 0 || nand->erasesize == 0) {
        MESSAGE("Failed to initialize NAND flash for saving Shmoo values!\n");
        return -1;
    }
    ...
    /* For NAND with bad blocks, we always erase all blocks in the range */
    {
        ...
        if (ret) {
            MESSAGE("NAND flash erase failed, error=%d\n", ret);
            return ret;
        }
    }
    ...
    while (offset < end) {
        ...
        if (ret) {
            MESSAGE("NAND flash write failed, error=%d\n", ret);
        }
        break;
    }
...
static int write_shmoo_to_flash(void *buf, int length)
{
    ...
    if (!flash) {
        MESSAGE("Failed to initialize SPI flash for saving Shmoo values!\n");
        return -1;
    }
    ...
    if (erase) {
        ...
        if (ret) {
            MESSAGE("SPI flash erase failed, error=%d\n", ret);
    ...
    if (ret) {
        MESSAGE("SPI flash write failed, error=%d\n", ret);
    }
...
static int write_shmoo_to_flash(void *buf, int length)
{
    ...
    if (erase) {
        ...
        if (ret) {
            MESSAGE("NOR flash erase failed, error=%d, start addr: 0x%x, end addr: 0x%x\n", 
    ...
    if (ret) {
        MESSAGE("NOR flash write failed, error=%d\n", ret);
    }
...
static int try_restore_shmoo(void)
{
    ...
    MESSAGE("Validate Shmoo parameters stored in flash ..... ");
    ...
    if (CONFIG_SHMOO_REUSE_DELAY_MSECS > 0) {
        char c = 0;
        unsigned long start;
        MESSAGE("Press Ctrl-C to run Shmoo ..... ");
        start = get_timer(0);
        while(get_timer(start) <= CONFIG_SHMOO_REUSE_DELAY_MSECS) {
            if (tstc()) {
                ...
                if (c == 0x03) {
                    MESSAGE("Pressed.\n");
                    MESSAGE("Do you want to run the Shmoo? [y/N] ");
                    for(;;) {
                        c = getc();
                        if (c == 'y' || c == 'Y') {
                            MESSAGE("Y\n");
    ...
    MESSAGE("Restoring Shmoo parameters from flash ..... ");
    ...
    if (CONFIG_SHMOO_REUSE_MEMTEST_LENGTH > 0 ) {
        MESSAGE("Running simple memory test ..... ");
...
void iproc_save_shmoo_values(void)
{
    ...
    if (buffer == NULL) {
        MESSAGE("Error allocating memory for saving Shmoo values!\n");
        return;
    }
    ptr = buffer;
    ...
    /* Write to flash */
    MESSAGE("Writing Shmoo values into flash .....\n");
...
static int clear_ddr(uint32_t offset, uint32_t size)
{
	...
	*(unsigned int *)(0x18010400) &= ~0x1;
	...
	{
		MESSAGE("clear_ddr: OK\n");
		return(0);
	}
	MESSAGE("clear_ddr: Failed: 0x%lx\n", get_timer(start));
	if(reg32_read((volatile uint32_t *)DDR_BistErrorOccurred))
		MESSAGE("clear_ddr: Error occured\n");
	return(1);
}
...
void ddr_init2(void)
{
	...
	for (i=8; i<=0xf; i++) {
		...
		MESSAGE("OTP offset(0x%x): 0x%x\n", i, otp_status);
	}
	if (is_ddr_32bit()) {
		MESSAGE("NSP25 32bit DDR\n");
	}
  else {
    MESSAGE("Not 32bit DDR\n");
  }
...
#if defined(CONFIG_TP_WOLFHOUND_BOARD)
    if( ddr_type) {  /* DDR3 */
        /* mhz = 0x190 (400MHz), 16 bit bus, 16 bit chip width, 1.50v vddq, 2Gbit chip size, jedec type = 10 (DDR3-800E) */
        reg32_write((volatile uint32_t *)DDR_PHY_CONTROL_REGS_STRAP_CONTROL, 0x01905555);
        val = reg32_read((volatile uint32_t *)DDR_PHY_CONTROL_REGS_REVISION);
		
        /* DRAM type = 1 i.e DDR3, AL = 0, write recovery = 6, write latency = 5, CAS read latency = 5 */
        reg32_write((volatile uint32_t *)DDR_PHY_CONTROL_REGS_STRAP_CONTROL2, 0x00106489);
        val = reg32_read((volatile uint32_t *)DDR_PHY_CONTROL_REGS_REVISION);

    }
	else {   /* DDR2 */
        /* mhz = 0x190 (400MHz), 16 bit bus, 16 bit chip width, 1.80v vddq, 1Gbit chip size, jedec type = 06 (DDR2-800E) */
        reg32_write((volatile uint32_t *)DDR_PHY_CONTROL_REGS_STRAP_CONTROL, 0x0190168d);
        val = reg32_read((volatile uint32_t *)DDR_PHY_CONTROL_REGS_REVISION);

        /* DRAM type = 0 i.e DDR2, AL = 0, write recovery = 6, write latency = 4, CAS read latency = 5 */
        reg32_write((volatile uint32_t *)DDR_PHY_CONTROL_REGS_STRAP_CONTROL2, 0x00006205);
        val = reg32_read((volatile uint32_t *)DDR_PHY_CONTROL_REGS_REVISION);
	}
#else
	/* mhz = 0x190, 16 bit bus, 16 bit chip width, 1.35v vddq, 8Gb chip size, jedec type = 25 (DDR3-1866M) */
	...
	MESSAGE("ddr_init2: Calling soc_ddr40_set_shmoo_dram_config\n");
#if (defined(CONFIG_HELIX4) || defined(CONFIG_KATANA2))
...
#error "DRAM config is not set"
	...
	MESSAGE("ddr_init2: Calling soc_ddr40_phy_calibrate\n");
    if(soc_ddr40_phy_calibrate(unit, 0, DDR_PHYTYPE_ENG, 0) != SOC_E_NONE) {

		MESSAGE("DDR phy calibration failed\n");
		return;
	}
...
ddr_cntrl_prog:
	...
	dump_phy_regs();
#endif
	MESSAGE("Programming controller register\n");
	ddr_init_regs(ddr_init_tab);
	if(ddr_type) {
		/* DDR3 */	
	...
	defined(CONFIG_KATANA2))
		switch(ddr_clk) {
			...
			case 400:
#if defined(CONFIG_TP_WOLFHOUND_BOARD)
				ddr_init_regs(ddr3_init_tab_800_tp_wolfhound_board);
#else
				ddr_init_regs(ddr3_init_tab_800);
#endif
				break;
		...
		switch(ddr_clk) {
			case 333:
#if defined(CONFIG_TP_WOLFHOUND_BOARD)
                ddr_init_regs(ddr2_init_tab_667_tp_wolfhound_board);
#else
			...
			case 400:
#if defined(CONFIG_TP_WOLFHOUND_BOARD)
                ddr_init_regs(ddr2_init_tab_800_tp_wolfhound_board);
#else
#if defined(CONFIG_TP_HELIX4_BOARD)
		  ddr_init_regs(ddr3_init_tab_800_tp_helix4_board);
#else
			...
			case 533:
#if defined(CONFIG_TP_WOLFHOUND_BOARD)
                ddr_init_regs(ddr2_init_tab_1066_tp_wolfhound_board);
#else
...
poll_ddr_ctrl:
	while(!(reg32_read((volatile uint32_t *)DDR_DENALI_CTL_89) & 0x100));
	MESSAGE("ddr_init2: Calling soc_ddr40_shmoo_ctl\n");
...
#if defined(CONFIG_IPROC_DDR_ECC)
	MESSAGE("Enabling DDR ECC reporting\n");
