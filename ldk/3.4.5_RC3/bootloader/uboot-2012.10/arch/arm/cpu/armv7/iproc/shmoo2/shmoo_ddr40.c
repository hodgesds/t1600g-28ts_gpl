    ...
    if (!stat) {
        switch(phyType) {
            ...
            /*    _soc_ddr40_arad_phy_init_mem_set_wrapper(unit, &ENGfgmss, freq_loc, grade_loc); */
                MESSAGE("C01. Check Power Up Reset_Bar\n");
                ...
                MESSAGE("C02. Config and Release PLL from reset\n");
                for (ciC = 0; ciC < MAX_SHMOO_INTERFACES; ciC+=2) { /* Only Even CIs */
                    if(!_check_dram_either(ciC)) {
                        continue;
                    }

					rval = reg32_read((volatile uint32_t *)IPROC_WRAP_IPROC_XGPLL_CTRL_0);
					//printf("IPROC_WRAP_IPROC_XGPLL_CTRL_0(0x1803fc1c) = %08x\n", rval);
					rval = reg32_read((volatile uint32_t *)IPROC_WRAP_IPROC_XGPLL_CTRL_1);
					//printf("IPROC_WRAP_IPROC_XGPLL_CTRL_1(0x1803fc20) = %08x\n", rval);
					rval = reg32_read((volatile uint32_t *)IPROC_WRAP_IPROC_XGPLL_CTRL_2);
					//printf("IPROC_WRAP_IPROC_XGPLL_CTRL_2(0x1803fc24) = %08x\n", rval);
					rval = reg32_read((volatile uint32_t *)IPROC_WRAP_IPROC_XGPLL_CTRL_4);
					//printf("IPROC_WRAP_IPROC_XGPLL_CTRL_4(0x1803fc2c) = %08x\n", rval);
					
                    /* Divider */
                    SOC_IF_ERROR_RETURN(READ_DDR40_PHY_CONTROL_REGS_PLL_DIVIDERSr(unit,ciC,&rval));
					//printf("DDR40_PHY_CONTROL_REGS_PLL_DIVIDERS(0x1801081c) = %08x\n", rval);
#if defined(CONFIG_TP_WOLFHOUND_BOARD)
					DDR40_SET_FIELD(rval,DDR40_PHY_CONTROL_REGS,PLL_DIVIDERS,NDIV,32); //keep default value
					DDR40_SET_FIELD(rval,DDR40_PHY_CONTROL_REGS,PLL_DIVIDERS,POST_DIV,2);
#else
...
#endif
                   // printf("Set DDR40_PHY_CONTROL_REGS_PLL_DIVIDERS(0x1801081c) = %08x\n", rval);
                ...
                MESSAGE("C04. Calibrate ZQ (ddr40_phy_calib_zq)\n");
                for (ciC = 0; ciC < MAX_SHMOO_INTERFACES; ciC+=2) { /* Only Even CIs */
                    if(!_check_dram_either(ciC)) {
                        continue;
                    }
                ...
                MESSAGE("C06. DDR40_PHY_DDR3_MISC\n");
                for (ciC = 0; ciC < MAX_SHMOO_INTERFACES; ciC+=2) { /* Only Even CIs */
                    if(!_check_dram_either(ciC)) {
                        continue;
                    }
            ...
            */
                ...
                if (!SAL_BOOT_QUICKTURN) {
					do {
                  uint32 rd_en_byte_mode=0, rd_en_byte_vdl_steps=0, rd_en_bit_vdl_offset=0;
                  uint32 calib_success = 1;

                    MESSAGE("C07. VDL Calibration\n");
                    for (ciC = 0; ciC < MAX_SHMOO_INTERFACES; ciC+=2) { /* Only Even CIs */
                        if(!_check_dram_either(ciC)) {
                            continue;
                        }
                    ...
                    for (ciC = 0; ciC < MAX_SHMOO_INTERFACES; ciC+=2) { /* Only Even CIs */
                        ...
                        //printf("VDL calibration result: 0x%0x (cal_steps = %d)\n", \
                         //          rval, DDR40_GET_FIELD(rval,  DDR40_PHY_CONTROL_REGS, VDL_CALIB_STATUS, CALIB_TOTAL) >> 4));
						//while(1); // Srini
                        MESSAGE("VDL calibration result: 0x%0x (cal_steps = %d)\n", rval, DDR40_GET_FIELD(rval,  DDR40_PHY_CONTROL_REGS, VDL_CALIB_STATUS, CALIB_TOTAL) >> 4);
                    ...
                    if( 1 == calib_success) {
                        ...
                        sal_usleep(100000); /* sleep 100ms, try to calibrate again */
                    }
                } while(1); }

					else {
                    MESSAGE("C07. VDL Calibration SKIPPED\n");
                }/* Not QUICKTURN */
                ...
                for (ciC = 0; ciC < MAX_SHMOO_INTERFACES; ciC+=2) { /* Only Even CIs */
                    ...
                    rval = 0;
                    DDR40_SET_FIELD(rval,DDR40_PHY_WORD_LANE_0,READ_CONTROL,DQ_ODT_ENABLE,0);
                    DDR40_SET_FIELD(rval,DDR40_PHY_WORD_LANE_0,READ_CONTROL,DQ_ODT_LE_ADJ,0);
                    DDR40_SET_FIELD(rval,DDR40_PHY_WORD_LANE_0,READ_CONTROL,DQ_ODT_TE_ADJ,0);
                ...
                MESSAGE("C09. Start ddr40_phy_autoidle_on (MEM_SYS_PARAM_PHY_AUTO_IDLE) ....\n");
                for (ciC = 0; ciC < MAX_SHMOO_INTERFACES; ciC+=2) { /* Only Even CIs */
                    if(!_check_dram_either(ciC)) {
                        continue;
                    }
                ...
                MESSAGE("C10. Wait for Phy Ready\n");
