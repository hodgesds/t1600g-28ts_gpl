...
void iproc_clk_enum()
{
	...
	if(iproc_clk.apb_clk < 1000000 || iproc_clk.arm_periph_clk < 1000000) {
		MESSAGE("arm_clk=%dHz, axi_clk=%dHz, apb_clk=%dHz, arm_periph_clk=%dHz\n", 
		iproc_clk.arm_clk, iproc_clk.axi_clk, 
		iproc_clk.apb_clk, iproc_clk.arm_periph_clk);
	}
	else {
		MESSAGE("arm_clk=%dMHz, axi_clk=%dMHz, apb_clk=%dMHz, arm_periph_clk=%dMHz\n", 
