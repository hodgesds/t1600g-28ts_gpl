...
bcmspi_disable_bspi(struct bcmspi_priv *priv)
{
    ...
    if ((priv->bspi_hw->mast_n_boot_ctrl & 1) == 0) {
        ...
        while(get_timer(start) < QSPI_WAIT_TIMEOUT * CONFIG_SYS_HZ / 1000) {
            ...
            /* Terminate the raf option of bspi by set START-bit(superset clear bit) to QSPI_raf_CTRL */
            priv->bspi_hw_raf->ctrl = BSPI_RAF_CONTROL_START_MASK;
            udelay(1000);
        }
        if (priv->bspi_hw->mast_n_boot_ctrl != 1) {
            printf("SF: BSPI is too busy to switch to MSPI, priv->state=%d\r\n", priv->state);
            return -1;
        }
    }
         ...
         void *din, unsigned long flags)
{
    ...
    if (!shutup)
    {
        int _i, _bytes = ((bytes >= 10)? 10:bytes);
        printf("%s, CMD(%d,dout=%#x,din=%#x):", __FUNCTION__, bytes, (int)dout, (int)din);
        if (dout)
        {
