...
static int soc_pcie_check_link(struct soc_pcie_port * port)
{
	...
	if( tmp8 != PCI_HEADER_TYPE_BRIDGE ) {
		MESSAGE("PCIe port %d in End-Point mode - ignored\n");
		linkError = 1;
		return -1;
	}
    else
        MESSAGE("PCIe port in RC mode\n");
	...
	MESSAGE("==>PCIE: LINKSTA reg %#x val %#x\n",
...
{
	if(conf_trace) MESSAGE("Pcie_rd_conf_dword: dev: %08x <B%d, D%d, F%d>, where: %08x\n", 
	...
	*value = __raw_readl(PCI_SLOT_ADDR + PCIE_CONF_DATA_OFF);
	...
	if(conf_trace) MESSAGE("Return : dev: %08x, where: %08x, val: %08x\n\n", dev, offset, *value);
...
int iproc_pcie_rd_conf_word(struct pci_controller *hose, pci_dev_t dev, int offset, u16 * value)
{
	...
	if(conf_trace) MESSAGE("Pcie_rd_conf_word: dev: %08x <B%d, D%d, F%d>, where: %08x\n", 
				dev, PCI_BUS(dev), PCI_DEV(dev), PCI_FUNC(dev), offset); 
	__raw_writel(PCIE_CONF_BUS( PCI_BUS(dev) ) |
					PCIE_CONF_DEV( PCI_DEV(dev) ) |
					PCIE_CONF_FUNC( PCI_FUNC(dev) ) |
    ...
    *value = (tmp >> (8 * (offset & 3))) & 0xffff;
	...
	if(conf_trace) MESSAGE("Return : dev: %08x, where: %08x, val: %04x\n\n", dev, offset, *value);
...
int iproc_pcie_rd_conf_byte(struct pci_controller *hose, pci_dev_t dev, int offset, u8 * value)
{
	...
	if(conf_trace) MESSAGE("Pcie_rd_conf_byte: dev: %08x <B%d, D%d, F%d>, where: %08x\n", 
				dev, PCI_BUS(dev), PCI_DEV(dev), PCI_FUNC(dev), offset); 
	__raw_writel(PCIE_CONF_BUS( PCI_BUS(dev) ) |
					PCIE_CONF_DEV( PCI_DEV(dev) ) |
					PCIE_CONF_FUNC( PCI_FUNC(dev) ) |
    ...
    *value = (tmp >> (8 * (offset & 3))) & 0xff;
	...
	if(conf_trace) MESSAGE("Return : dev: %08x, where: %08x, val: %02x\n\n", dev, offset, *value);
...
int iproc_pcie_wr_conf_dword(struct pci_controller *hose, pci_dev_t dev, int offset, u32 value)
{
	...
	if(conf_trace) MESSAGE("Pcie_wr_conf_dword: dev: %08x <B%d, D%d, F%d>, where: %08x, val: %08x\n", 
...
int iproc_pcie_wr_conf_word(struct pci_controller *hose, pci_dev_t dev, int offset, u16 value)
{
	...
	if(conf_trace) MESSAGE("Pcie_wr_conf_word: dev: %08x <B%d, D%d, F%d>, where: %08x, val: %04x\n", 
				dev, PCI_BUS(dev), PCI_DEV(dev), PCI_FUNC(dev), offset, value); 
	__raw_writel(PCIE_CONF_BUS( PCI_BUS(dev) ) |
					PCIE_CONF_DEV( PCI_DEV(dev) ) |
					PCIE_CONF_FUNC( PCI_FUNC(dev) ) |
...
int iproc_pcie_wr_conf_byte(struct pci_controller *hose, pci_dev_t dev, int offset, u8 value)
{
	...
	if(conf_trace) MESSAGE("Pcie_wr_conf_byte: dev: %08x <B%d, D%d, F%d>, where: %08x, val: %02x\n", 
				dev, PCI_BUS(dev), PCI_DEV(dev), PCI_FUNC(dev), offset, value); 
	__raw_writel(PCIE_CONF_BUS( PCI_BUS(dev) ) |
					PCIE_CONF_DEV( PCI_DEV(dev) ) |
					PCIE_CONF_FUNC( PCI_FUNC(dev) ) |
...
{
	if(conf_trace) MESSAGE("pci_read_config_dword: dev: %08x <B%d, D%d, F%d>, where: %08x\n", 
...
int pci_read_config_word (pci_dev_t dev, int where, unsigned short *val)
{
	...
	if(conf_trace) MESSAGE("pci_read_config_word: dev: %08x <B%d, D%d, F%d>, where: %08x\n", 
				dev, PCI_BUS(dev), PCI_DEV(dev), PCI_FUNC(dev), where); 
	__raw_writel(PCIE_CONF_BUS( PCI_BUS(dev) ) |
					PCIE_CONF_DEV( PCI_DEV(dev) ) |
					PCIE_CONF_FUNC( PCI_FUNC(dev) ) |
...
int pci_read_config_byte (pci_dev_t dev, int where, unsigned char *val)
{
	...
	if(conf_trace) MESSAGE("pci_read_config_byte: dev: %08x <B%d, D%d, F%d>, where: %08x\n", 
				dev, PCI_BUS(dev), PCI_DEV(dev), PCI_FUNC(dev), where); 
	__raw_writel(PCIE_CONF_BUS( PCI_BUS(dev) ) |
					PCIE_CONF_DEV( PCI_DEV(dev) ) |
					PCIE_CONF_FUNC( PCI_FUNC(dev) ) |
...
{
    unsigned int tmp;
	if(conf_trace) MESSAGE("pci_bus0_read_config_word: dev: %08x <B%d, D%d, F%d>, where: %08x\n", 
				dev, PCI_BUS(dev), PCI_DEV(dev), PCI_FUNC(dev), where);
...
{
    unsigned int tmp;
	if(conf_trace) MESSAGE("pci_bus0_read_config_byte: dev: %08x <B%d, D%d, F%d>, where: %08x\n", 
				dev, PCI_BUS(dev), PCI_DEV(dev), PCI_FUNC(dev), where);
...
int pci_write_config_dword (pci_dev_t dev, int where, unsigned int val)
{
	...
	if(conf_trace) MESSAGE("pci_write_config_dword: dev: %08x <B%d, D%d, F%d>, where: %08x, val: %08x\n", 
...
int pci_write_config_word (pci_dev_t dev, int where, unsigned short val)
{
	...
	if(conf_trace) MESSAGE("pci_write_config_word: dev: %08x <B%d, D%d, F%d>, where: %08x, val: %04x\n", 
				dev, PCI_BUS(dev), PCI_DEV(dev), PCI_FUNC(dev), where, val); 
	__raw_writel(PCIE_CONF_BUS( PCI_BUS(dev) ) |
					PCIE_CONF_DEV( PCI_DEV(dev) ) |
					PCIE_CONF_FUNC( PCI_FUNC(dev) ) |
...
int pci_write_config_byte (pci_dev_t dev, int where, unsigned char val)
{
	...
	if(conf_trace) MESSAGE("pci_write_config_byte: dev: %08x <B%d, D%d, F%d>, where: %08x, val: %02x\n", 
				dev, PCI_BUS(dev), PCI_DEV(dev), PCI_FUNC(dev), where, val); 
	__raw_writel(PCIE_CONF_BUS( PCI_BUS(dev) ) |
					PCIE_CONF_DEV( PCI_DEV(dev) ) |
					PCIE_CONF_FUNC( PCI_FUNC(dev) ) |
...
void pci_bus0_write_config_word (pci_dev_t dev, int where, unsigned short val)
{
	...
	if(conf_trace) MESSAGE("pci_bus0_write_config_word: dev: %08x <B%d, D%d, F%d>, where: %08x, val: %04x\n", 
				dev, PCI_BUS(dev), PCI_DEV(dev), PCI_FUNC(dev), where, val);
	...
	if(conf_trace) MESSAGE("pci_bus0_write_config_word read first: dev: %08x <B%d, D%d, F%d>, where: %08x, tmp_val: %04x\n", 
				dev, PCI_BUS(dev), PCI_DEV(dev), PCI_FUNC(dev), where, tmp);
    tmp &= ~(0xffff << (8 * (where & 3)) );
    tmp |= (val << (8 * (where & 3)) );

	if(conf_trace) MESSAGE("pci_bus0_write_config_word write back: dev: %08x <B%d, D%d, F%d>, where: %08x, tmp_val: %04x\n", 
				dev, PCI_BUS(dev), PCI_DEV(dev), PCI_FUNC(dev), where, tmp);
	__raw_writel( where & 0xffc, PCI_SLOT_ADDR + SOC_PCIE_EXT_CFG_ADDR );
	__raw_writel(tmp, PCI_SLOT_ADDR + SOC_PCIE_EXT_CFG_DATA);

    if(conf_trace) MESSAGE("pci_bus0_write_config_word write done\n");
}
...
void pci_bus0_write_config_byte (pci_dev_t dev, int where, unsigned char val)
{
	...
	if(conf_trace) MESSAGE("pci_bus0_write_config_byte: dev: %08x <B%d, D%d, F%d>, where: %08x, val: %02x\n", 
				dev, PCI_BUS(dev), PCI_DEV(dev), PCI_FUNC(dev), where, val);
...
#define   SZ_48M    (0x100000 * 48)
	...
	if(conf_trace) MESSAGE("==>pcieSwitchInit: dev: %08x <B%d, D%d, F%d>",
    ...
    if(conf_trace) MESSAGE("PCIE: Doing PLX switch Init...Test Read = %08x\n",(unsigned int)dRead);
...
/***/
	...
	if(conf_trace) MESSAGE("PCIE: Doing PLX switch Init...GPIO Read = %08x\n",(unsigned int)dRead);
	...
	if(PCI_BUS(busdevfn) == 1)
	{
		...
		//MESSAGE("==>pcieSwitchInit: 0x18 Readout %x\n", u32Readout);
		...
		//MESSAGE("==>pcieSwitchInit: PCI_MEMORY_BASE Readout %x\n", u32Readout);
...
static void pcieWIFIdeviceInit( pci_dev_t busdevfn)
{
	...
	if(conf_trace) MESSAGE("SATA Device Init: dev: %08x <B%d, D%d, F%d>",
			busdevfn, PCI_BUS(busdevfn), PCI_DEV(busdevfn), PCI_FUNC(busdevfn));
...
void pci_iproc_init_board (struct pci_controller * hose)
{
	...
	MESSAGE("PCI:   Bus Dev VenId DevId Class Int\n");
#endif
#if defined(CONFIG_NS_PLUS)
	}
#endif

	MESSAGE("Done PCI initialization\n");
...
struct pci_controller * hose;
	...
	MESSAGE("      Memory Base: %08x\n", tmp32 & 0xFFFF);
	MESSAGE("      Memory Limit: %08x\n", (tmp32 >> 16) & 0xFFFF );
...
static void pci_dump_standard_conf(struct soc_pcie_port * port)
{
    ...
    {      
		...
		cnt = i % 4;
		if(cnt==0) MESSAGE("i=%d <%x> \t 0x%08x \t", i, i, val);
		else if(cnt==3) MESSAGE("0x%08x \n", val);
		else MESSAGE("0x%08x \t", val);
	}
	MESSAGE("\n");
...
static void pci_dump_extended_conf(struct soc_pcie_port * port)
{
    ...
    for(i=0; i<128; i++)
    {
		...
		cnt = i % 8;
		if(cnt==0) MESSAGE("i=%d <%x> \t 0x%04x  ", i, i, val16);
		else if(cnt==7) MESSAGE("0x%04x \n", val16);
		else MESSAGE("0x%04x  ", val16);
	}
	MESSAGE("\n");
}
