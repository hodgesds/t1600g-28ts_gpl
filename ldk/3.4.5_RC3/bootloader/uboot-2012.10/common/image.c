...
void image_print_contents(const void *ptr)
{
	...
	MESSAGE("%sImage Name:   %.*s\n", p, IH_NMLEN, image_get_name(hdr));
#if defined(CONFIG_TIMESTAMP) || defined(CONFIG_CMD_DATE) || defined(USE_HOSTCC)
	MESSAGE("%sCreated:      ", p);
	genimg_print_time((time_t)image_get_time(hdr));
#endif
	MESSAGE("%sImage Type:   ", p);
	image_print_type(hdr);
	MESSAGE("%sData Size:    ", p);
	genimg_print_size(image_get_data_size(hdr));
	MESSAGE("%sLoad Address: %08x\n", p, image_get_load(hdr));
	MESSAGE("%sEntry Point:  %08x\n", p, image_get_ep(hdr));
...
static void genimg_print_time(time_t timestamp)
{
	...
	to_tm(timestamp, &tm);
	MESSAGE("%4d-%02d-%02d  %2d:%02d:%02d UTC\n",
			tm.tm_year, tm.tm_mon, tm.tm_mday,
			tm.tm_hour, tm.tm_min, tm.tm_sec);
#else
	MESSAGE("%s", ctime(&timestamp));
#endif
}
...
void fit_image_print(const void *fit, int image_noffset, const char *p)
{
	...
	MESSAGE("%s  Compression:  %s\n", p, genimg_get_comp_name(comp));

	ret = fit_image_get_data(fit, image_noffset, &data, &size);

#ifndef USE_HOSTCC
	MESSAGE("%s  Data Start:   ", p);
	if (ret)
		MESSAGE("unavailable\n");
	else
		MESSAGE("0x%08lx\n", (ulong)data);
