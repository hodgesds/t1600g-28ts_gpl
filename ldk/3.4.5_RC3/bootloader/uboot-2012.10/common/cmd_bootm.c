...
static int bootm_load_os(image_info_t os, ulong *load_end, int boot_progress)
{
	...
	case IH_COMP_GZIP:
		...
		if (gunzip((void *)load, unc_len,
			...
			MESSAGE("GUNZIP: uncompress, out-of-mem or overwrite "
	...
	case IH_COMP_BZIP2:
		...
		if (i != BZ_OK) {
			MESSAGE("BUNZIP2: uncompress or overwrite error %d "
	...
	case IH_COMP_LZMA: {
		...
		unc_len = lzma_len;
		if (ret != SZ_OK) {
			MESSAGE("LZMA: uncompress or overwrite error %d "
				"- must RESET board to recover\n", ret);
			bootstage_error(BOOTSTAGE_ID_DECOMP_IMAGE);
			return BOOTM_ERR_RESET;
		}
	...
	case IH_COMP_LZO:
		...
		ret = lzop_decompress((const unsigned char *)image_start,
					  ...
					  &unc_len);
		if (ret != LZO_E_OK) {
			MESSAGE("LZO: uncompress or overwrite error %d "
			      "- must RESET board to recover\n", ret);
			if (boot_progress)
				bootstage_error(BOOTSTAGE_ID_DECOMP_IMAGE);
			return BOOTM_ERR_RESET;
...
#endif /* CONFIG_LZO */
	default:
		MESSAGE("Unimplemented compression type %d\n", comp);
...
static image_header_t *image_get_kernel(ulong img_addr, int verify)
{
	...
	if (!image_check_target_arch(hdr)) {
		MESSAGE("Unsupported Architecture 0x%x\n", image_get_arch(hdr));
		...
		ulong *os_len)
{
	...
	*os_data = *os_len = 0;
	...
	case IMAGE_FORMAT_LEGACY:
		MESSAGE("## Booting kernel from Legacy Image at %08lx ...\n",
				img_addr);
